...
test_that("validate_inputs_graph warks not correct", {
  expect_error(
    validate_inputs_graph(peaks = letters),
    "'peaks' must be a GRanges object"
  )
  expect_error(
    validate_inputs_graph(peaks = peaks, annoData = letters),
    "'annoData' must be an annoGR or GRanges object"
  )
  expect_error(
    validate_inputs_graph(peaks = peaks, annoData = GRanges()),
    "'annoData' is empty."
  )
  expect_error(
...
test_that("build_interaction_graph or detect_clusters works not correct", {
  ...
  expect_equal(
    res[[1]]$clusters["g2_1", "cluster_id"],
    res[[2]]$clusters["g1_1", "cluster_id"]
  )
  expect_equal(
    res[[2]]$clusters["g2_1", "cluster_id"],
  ...
  expect_equal(
    res[[3]]$clusters["g2_1", "cluster_id"],
